...
public class HomeController {
	...
	/*
	 * Service 클래스에 @Service 어노테이션을 붙인후 
	 * DAO 클래스는 @Repository 어노테이션
	 * Controller 클래스에서 @Autowired를 붙여서 Service
	 * 클래스 객체를 선언하면 //매번 new 를 이용하여 객체를 만들지 않고도 Service 클래스의 자원(필드,메소드)을 활용할 수 있음.
	 */

	
	/*
	 * //TestService에 있는 abc라는 메소드를 호출하고 싶다면
	 * TestService tts=new TestService(); tts.abc() ;
	 */ 
	...
	public void welcomemsg(@RequestParam("msg") String msg, @RequestParam("msg") String msg2) {
	...
	public void insertDB(@RequestParam("data1") String data1) {
		...
		// src/main/java/com/icia/example/ 이하에 dao,service 폴더가 같은 수준으로 보여야함.
		
		//TestService 클래스에 있는 method1 호출
		
		/*
		 * hello.jsp 에서 입력하고 가장 버튼 클릭  
		 * HomeController의 insertDB 메소드 호출되고
		 * 여기서 TestService의 method1 메소드 호출함
		 * hello.jsp->HomeController->TestService->TestDAO->DataBase 과정으로 진행
		 */		
		ts.method1();
